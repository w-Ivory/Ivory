    ...
    **/
    public function getMessage( $conversation = NULL ) {
        try {
            ...
            return $this->executeQuery( 'SELECT `c_id` AS "Conversation", `m_id` AS "ID", DATE_FORMAT( `m_date`, "%d/%m/%Y" ) AS "Date", DATE_FORMAT( `m_date`, "%H:%i:%s" ) AS "Heure", CONCAT( `u_prenom`, " ", `u_nom` ) AS "Auteur", `m_contenu` AS "Message" FROM `conversation` LEFT JOIN `message` ON `c_id`=`m_conversation_fk` LEFT JOIN `user` ON `m_auteur_fk`=`u_id` ' . ( $this->is_valid_int( $conversation ) ? ' WHERE `c_id`=:id' : '' ) . ' ORDER BY `m_date` DESC', $options );
        } catch( CoreException $e ) {
            throw $e;
        } catch( PDOException $e ) {
            throw new CoreException( $e->getMessage() );
