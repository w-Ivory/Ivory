...
<section role="main" id="villes">
    <article role="article">
        ...
        <ul class="grid-wrapper widgets">
            <li class="grid12 x12 item before" data-id="villes">
                <?php
                $_str_query = 'SELECT `ville`.`v_id` AS "ID", `ville`.`v_nom` AS "Nom", `ville`.`v_superficie` AS "Superficie"
                FROM `ville`
                ORDER BY `ville`.`v_nom` ASC'; // On définit la chaîne de caractères représentant la requête.
                ...
                <div class="table">
                    ...
                    <div class="tbody">
                        ...
                        <div class="tr">
                            ...
                            <?php endforeach; ?>
                            <div class="td"><a class="link" href="<?php echo ( defined( 'DOMAIN' ) ? DOMAIN : '' ); ?>ville.php?id=<?php echo $item['ID']; ?>" title="Voir la fiche">Voir la fiche</a></div>
                        </div>
                        <?php endforeach; ?>
                    </div>
                </div>
