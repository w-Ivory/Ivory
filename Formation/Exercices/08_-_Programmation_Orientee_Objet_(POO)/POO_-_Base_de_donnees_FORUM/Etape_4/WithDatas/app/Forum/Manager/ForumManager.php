    ...
    **/
    public function getConversation( $id = NULL ) {
        try {
            ...
            return $this->executeQuery( 'SELECT `c_id` AS "ID", DATE_FORMAT( `c_date`, "%d/%m/%Y" ) AS "Date", DATE_FORMAT( `c_date`, "%H:%i:%s" ) AS "Heure", COUNT( `m_id` ) AS "Total Message(s)", `c_termine` AS "Status" FROM `conversation` LEFT JOIN `message` ON `c_id`=`m_conversation_fk`' . ( $this->is_valid_int( $id ) ? ' WHERE `c_id`=:id' : '' ) . ' GROUP BY `c_id` ORDER BY `c_id`', $options );
        } catch( CoreException $e ) {
            throw $e;
        } catch( PDOException $e ) {
            throw new CoreException( $e->getMessage() );
