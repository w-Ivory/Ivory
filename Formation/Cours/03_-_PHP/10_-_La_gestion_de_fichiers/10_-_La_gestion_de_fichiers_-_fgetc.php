...
<html>
    <head>
        ...
        <title>La gestion de fichiers - Lecture caractère par caractère via fgetc()</title>
    ...
    <body>
        <h1>La gestion de fichiers - Lecture caractère par caractère via fgetc()</h1>
        <a href="../10_-_La_gestion_de_fichiers.php" style="color:rgb(0,0,0);" title="">&larr; Retour au cours</a>
        <hr />
        <h2>Lecture caractère par caractère du fichier <?php echo $cheminFichier; ?> :</h2>
        <p>
        <?php
        if( file_exists( $cheminFichier ) ) :
            $ressourceFichier = fopen( $cheminFichier, 'r' );
            echo fgetc( $ressourceFichier );
...
if( file_exists( $cheminFichier ) ) : // Si le fichier existe,
    ...
    echo fgetc( $ressourceFichier ); // On affiche le contenu de la ligne désignée par le curseur dans le fichier (http://php.net/manual/fr/function.fgets.php).
