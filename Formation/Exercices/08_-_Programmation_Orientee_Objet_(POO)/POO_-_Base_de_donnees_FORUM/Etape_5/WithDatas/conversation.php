...
try {
    ...
    if( SRequest::getInstance()->get( 'sort' )!==NULL ) :
        switch( SRequest::getInstance()->get( 'sort' ) ) :
            case 'DESC':
                $current_order = 'DESC';
                break;
            default:
                $current_order = 'ASC';
        endswitch;
        // SRequest::getInstance()->unset( 'get', 'sort' );
        unset( $get['sort'] );
    else :
        $current_order = 'ASC';
    endif;
    if( SRequest::getInstance()->get( 'sortby' )!==NULL ) :
        switch( SRequest::getInstance()->get( 'sortby' ) ) :
            case 'auteur' :
                $order_string = ' ORDER BY `u_prenom` ' . $current_order . ', `u_nom` ' . $current_order;
                $current_sort = 'auteur';
                break;
            default :
                $order_string = ' ORDER BY `m_date` ' . $current_order;
                $current_sort = 'date';
        endswitch;
        // SRequest::getInstance()->unset( 'get', 'sortby' );
        unset( $get['sortby'] );
    else :
        $order_string = ' ORDER BY `m_date` ' . $current_order;
        $current_sort = 'date';
    ...
    $datas = $forumManager->getMessage( SRequest::getInstance()->get( 'conv' ), $order_string, ( ( $current_page - 1 ) * RESULTS_PER_PAGE ) , RESULTS_PER_PAGE );
                    ...
                    <tr>
                        ...
                        foreach( $datas[0] as $key=>$value ) :
                            switch( $key ) :
                                ...
                                case 'Date':
                                case 'Auteur':
                                    echo '<th>' . $key . ' (<a class="tri' . ( isset( $current_order ) && $current_sort==strtolower( $key ) ? ' actif ' . strtolower( $current_order ) : '' ) . '" href="' . ( count( $get )>0 ? '?' . http_build_query( $get ) . '&sortby=' . strtolower( $key ) . '&sort=' . ( $current_order=='ASC' ? 'DESC' : 'ASC' ) : '?sortby=' . strtolower( $key ) . '&sort=' . ( $current_order=='ASC' ? 'DESC' : 'ASC' ) ) . '" title="">Changer tri</a>)</th>';
                                    break;
                                default:
                                    echo '<th>' . $key . '</th>';
                                    break;
