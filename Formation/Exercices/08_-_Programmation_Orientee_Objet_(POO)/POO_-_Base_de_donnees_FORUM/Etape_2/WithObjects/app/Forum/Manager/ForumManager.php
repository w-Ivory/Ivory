    ...
    **/
    public function getConversation( $id = NULL ) {
        try {
            ...
            return $this->executeQuery( 'SELECT `c_id` AS "ID", DATE_FORMAT( `c_date`, "%d/%m/%Y" ) AS "Date", DATE_FORMAT( `c_date`, "%H:%i:%s" ) AS "Heure", COUNT( `m_id` ) AS "Total Message(s)", `c_termine` AS "Status"' . ( $this->is_valid_int( $id ) ? ', GROUP_CONCAT( DISTINCT CONCAT( "ID", "_:_", `m_id`, "_,_", "Date", "_:_", DATE_FORMAT( `m_date`, "%d/%m/%Y" ), "_,_", "Heure", "_:_", DATE_FORMAT( `m_date`, "%H:%i:%s" ), "_,_", "Auteur", "_:_", `u_prenom`, " ", `u_nom`, "_,_", "Message", "_:_", `m_contenu` ) ORDER BY `m_date` DESC SEPARATOR "_;_") AS "Message"' : '' ) . ' FROM `conversation` JOIN `message` ON `c_id`=`m_conversation_fk`' . ( $this->is_valid_int( $id ) ? ' LEFT JOIN `user` ON `m_auteur_fk`=`u_id` WHERE `c_id`=:id' : '' ) . ' GROUP BY `c_id` ORDER BY `c_id`', $options );
        } catch( CoreException $e ) {
            throw $e;
        } catch( PDOException $e ) {
            throw new CoreException( $e->getMessage() );
