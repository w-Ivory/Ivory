 ...
 * ------------------------------------------------------------
 * CLASS CONVERSATION
 * (Requires : TypeTest | NavigationManagement | SRequest | ClassMessage)
 * ------------------------------------------------------------
**/
class ClassConversation {
    ...
    **/
    protected function setDate( $value ) {
        if( TypeTest::is_valid_date( $value, 'd/m/Y' ) )
            $this->_date = $value;
    }
    ...
    **/
    protected function setHeure( $value ) {
        if( TypeTest::is_valid_date( $value, 'H:i:s' ) )
            $this->_heure = $value;
    }
    ...
    **/
    protected function setPagination() {
        if( SRequest::getInstance()->get( 'page' )!==NULL && TypeTest::is_valid_int( SRequest::getInstance()->get( 'page' ) ) && SRequest::getInstance()->get( 'page' )>0 ) :
        ...
        if( TypeTest::is_valid_int( $current_page ) && TypeTest::is_valid_int( static::$_limit ) && $current_page>=0 && static::$_limit>0 )
    ...
    **/
    public function getExcerpt() {
        ...
        $str .= '<td>' . ( $this->getId()!==NULL ? '<a class="more" href="?' . http_build_query( SRequest::getInstance()->get() ) . '&a=conversation&conv=' . $this->getId() . '" title="">Voir les messages</a>' : '' ) . '</td></tr>';
    ...
    **/
    protected function getPagination() {
        if( isset( $this->_pagination['CURRENT_PAGE'] ) && TypeTest::is_valid_int( $this->_pagination['CURRENT_PAGE'] ) && isset( $this->_pagination['RESULTS_PER_PAGE'] ) && TypeTest::is_valid_int( $this->_pagination['RESULTS_PER_PAGE'] ) && $this->_pagination['CURRENT_PAGE']>=0 && $this->_pagination['RESULTS_PER_PAGE']>0 )
            return '
        <tr>
